...
\begin{algorithm}[ht]
    ...
    $f \gets 0$ \\
	$E^f \gets E \cup \{ rev(e) \mid e\in E \}$ \\
    $G^f \gets (V, E^f)$ \tcp{residual graph}
    \While{$t$ reachable from $s$ in $G^f$} {
	    Choose an $(s,t)$ path in $G^f$ and maximize flow along the edges of the path \tcp{reverse flow cancels existing flow}
	    $c^f \gets \begin{cases}
	    c_{uv} - f_{uv} & [(u,v)\in E \wedge f_{uv}<c_{uv}] \\
	    f_{vu} & [(v,u)\in E \wedge f_{vu}>0]
	    \end{cases}$
    }
    \Return{$f$}
...
\item Dual LP --- $\underset{\vec{y}}{\text{argmin}}\, \{\vec{b}^{\,\textsf{T}} \vec{y} \mid A^\textsf{T}\vec{y} = \vec{c}, \, \vec{y} \geq 0 \}$
\end{itemize}
Then, 
\begin{itemize}
